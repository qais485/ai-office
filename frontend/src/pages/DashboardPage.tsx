    ...
    <li className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${AGENT_STATUS_COLORS[agent.status] ?? 'bg-white/10 text-white/50'}`}>
            {agent.status}
          </span>
        </div>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-xs text-white/50 mt-0.5">
            {task.agent_name}
            <span className={`ml-2 ${PRIORITY_COLORS[task.priority] ?? 'text-white/50'}`}>
              {task.priority}
            </span>
          </p>
        </div>
        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0 ${TASK_STATUS_COLORS[task.status] ?? 'bg-white/10 text-white/70'}`}>
...
function EmptyState({ message, detail }: { message: string; detail: string }) {
  return (
    <div className="text-center py-6">
      <svg className="mx-auto h-8 w-8 text-white/25" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor">
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white/[0.03] rounded-xl border border-white/10 p-6">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="bg-white/[0.03] rounded-xl border border-white/10">
      ...
      {summary.recent_tasks.length > 0 && (
        <section className="mt-6 bg-white/[0.03] rounded-xl border border-white/10">
          <div className="px-5 py-4 border-b border-white/6">
